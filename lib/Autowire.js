...
export default class Autowire {
  ...
  constructor(container, tsConfigFullPath = null) {
    ...
    try {
      this._tsConfigFullPath =
        tsConfigFullPath || path.join(process.cwd(), "tsconfig.json");
  ...
  *_walkFilePath(dir, fileName) {
  ...
  async _getServiceIdFromPath(path, type) {
    if (path) {
      ...
      if (namespaceDepth && namespaceDepth > 0) {
  ...
  async _executeFilePath(filePath) {
    ...
    const { classDeclaration, body } = await this._getClassDeclaration(
      filePath
    );
    if (!classDeclaration) {
      return;
    ...
    await this._interfaceImplementations(
      classDeclaration,
      body,
      parsedFile,
      serviceId
  ...
  async _getClassDeclaration(filePath) {
    ...
    const classDeclaration = body.find((declaration) => {
  ...
  _rebuildFilePath(parsedFile) {
    return parsedFile.dir + path.sep + parsedFile.base;
  }
  /**
   *
  ...
  async _getDefinition(classDeclaration, body, parsedFile, ServiceClass) {
    try {
      ...
      for (const parameterDeclaration of constructorParams) {
        let typeNameForArgument =
          parameterDeclaration.parameter.typeAnnotation.typeAnnotation.typeName;
   ...
   * @returns
   */
  async _getConstructorParamsByDefinition(
    definition,
    classDeclaration,
    body,
    parsedFile
    ...
    const constructorDeclaration =
      declarationWithNameSpace.declaration.body.body.find(
   ...
   */
  async _interfaceImplementations(
    classDeclaration,
    body,
    parsedFile,
    serviceId
   ...
   * @returns {Promise}
   */
  async _getIdentifierFromImports(
    type,
    body,
    parsedFile,
    importNameForAgument = null
  ) {
    try {
      let rootDir = parsedFile.dir;
      let relativeImportForImplement = await this._getRelativeImport(
        body,
        type,
        importNameForAgument
      );
      ...
      for (const pathConfig in configPaths) {
        ...
        const tsRelativePath = this._tsConfigPaths[pathConfig][0].replace(
          /\*/g,
          ""
        );
      ...
      let absolutePathImportForImplement;
      if (type.indexOf("ref__") === -1) {
        absolutePathImportForImplement = path.join(
          rootDir,
          relativeImportForImplement
        );
      }
      if (parsedFile.base.indexOf("AuthService") !== -1) {
        this._getServiceIdFromPath(absolutePathImportForImplement, type);
      }
  ...
  async _getRelativeImport(body, type, importNameForAgument = null) {
    const declaration = body.find((declaration) => {
